import { createState, isBasePtr, isStateful, Stateful, stateListen } from "./state";

let delegates = [];

export function createStore<T extends Object>(target: T, options: {
	ident: string,
	backing: "localstorage" | { read: (ident: string) => string, write: (ident: string, data: string) => void },
	autosave: "auto" | "manual" | "beforeunload"
}): Stateful<T> {
	let { ident, backing, autosave } = options;
	let read: (ident: string) => string, write: (ident: string, data: string) => void;
