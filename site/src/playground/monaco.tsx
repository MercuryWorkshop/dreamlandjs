...
let dreamland = dreamlandFiles.map(([path, content]) => ({ content, filePath: monaco.Uri.file(path).toString() }));
...
type TypeScriptWorkerInit = (...uris: monaco.Uri[]) => Promise<monaco.languages.typescript.TypeScriptWorker>;
...
export let Monaco: Component<{ value: string, transpiled: string, }> = function(cx) {
