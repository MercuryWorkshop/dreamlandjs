export let title = ["Migrating", "From v0.0.x"];
...
`$if(x, yes, no)` is now `use(x).and(yes, no).or()`
...
Many items that were once accessed by `this.` are now accessed by `this.cx.` and JSX.Element is replaced with ComponentChild
