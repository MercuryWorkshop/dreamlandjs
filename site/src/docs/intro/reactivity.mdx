...
let sum = use(counters.one).zip(use(counters.two), use(counters.three))
	.map(([one, two, three]) => one + two + three);

let average = use(counters.one, counters.two, counters.three)
	.map(([one, two, three]) => (one + two + three) / 3);
```
