...
dreamland targets **Chrome 88**, **Firefox 79**, and **Safari 14.1** (due to usage of `:where()` in scoped CSS and `WeakRef`s in the state system). Transpilers and polyfills can be used to run dreamland on older versions, but scoped CSS will break.
...
This just sets the following vite oxc config:
