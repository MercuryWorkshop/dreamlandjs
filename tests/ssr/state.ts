...
ssrTest(
	"ssr/state: scalars come from the server, not the client body",
	async () => {
...
ssrTest(
	"KNOWN BUG ssr/state: a null value does not crash serialization",
	async () => {
...
ssrTest(
	"KNOWN BUG ssr/state: undefined does not come back as null",
	async () => {
...
ssrTest("ssr/state: nested plain objects round-trip", async () => {
	let { App, toClient } = sided((side) => ({
		cfg:
			side === "server"
				? { a: 1, deep: { b: "two" } }
...
ssrTest("ssr/state: arrays, maps and sets round-trip", async () => {
	let { App, toClient } = sided((side) => ({
...
ssrTest(
	"ssr/state: [NO_CHANGE] opts an object out of serialization",
	async () => {
		let App = function (this: any) {
			this[NO_CHANGE] = true;
...
ssrTest("ssr/state: dom nodes and functions are not serialized", async () => {
	let App = function (this: any) {
		this.fn = () => "nope";
		this.node = jsx("div", {});
		this.keep = "yes";
...
ssrTest("ssr/state: values are interned rather than repeated", async () => {
...
ssrTest(
	"KNOWN BUG ssr/state: cyclic state does not crash the render",
	async () => {
...
ssrTest(
	"KNOWN BUG ssr/state: shared references keep their identity",
	async () => {
...
ssrTest(
	"ssr/state: props passed down are still readable after hydration",
	async () => {
		let Child = function (this: any) {
