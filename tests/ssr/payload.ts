...
ssrTest("ssr/payload: state cannot break out of the data script", async () => {
...
ssrTest(
	"ssr/payload: hostile state still hydrates to the original string",
	async () => {
		let side = "server";
		let App = function (this: any) {
...
ssrTest(
	"ssr/payload: quotes, newlines and astral characters survive",
	async () => {
		let tricky =
...
ssrTest(
	"ssr/payload: text content is escaped in markup, not injected",
	async () => {
		let App = function () {
...
ssrTest(
	"ssr/payload: text and comment entries point at their real parent",
	async () => {
		let App = function () {
			return jsx("main", {
...
ssrTest(
	"KNOWN BUG ssr/payload: a null child serializes as an empty comment",
	async () => {
...
ssrTest("ssr/payload: no entry is emitted for plain elements", async () => {
