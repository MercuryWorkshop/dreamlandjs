...
ssrTest("init runs on the server, mount does not", async () => {
	let calls: string[] = [];
...
ssrTest("mount runs once on the client, after hydration", async () => {
	let calls: string[] = [];
	let rootAtMount: any;
	let App = function (this: any) {
		this.cx.init = () => void calls.push("init");
...
ssrTest("synchronous init state reaches the client dom", async () => {
	let App = function (this: any) {
		this.text = "default";
		this.cx.init = () => {
			this.text = "from init";
...
ssrTest("state written after an await in init round-trips", async () => {
...
ssrTest("async init is awaited before render resolves", async () => {
	let App = function (this: any) {
		this.text = "pending";
		this.cx.init = async () => {
...
ssrTest("nested component init ordering is depth-first", async () => {
	let order: string[] = [];
...
ssrTest("state shared across components hydrates once", async () => {
